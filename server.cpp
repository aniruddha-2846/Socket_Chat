...
int main()
{
    ...
    if ((serverfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == ERROR)
    ...
    if (bind(serverfd, (struct sockaddr *)&for_server, sizeof(for_server)) == ERROR)
    ...
    if ((clientfd = accept(serverfd, (struct sockaddr *)&for_peer, &ipaddr_len)) == ERROR)
        servererror("accept() Error :: ");
    
    sandesh_info("Connection Established. ");
    
    inet_ntop(AF_INET, &(for_peer.sin_addr), ipaddress, INET_ADDRSTRLEN);
    std::cout << "IPaddress of Client :: "<<ipaddress<<std::endl<<std::endl;
    ...
    while (1)
    {
        if (recv(clientfd, buff_to_recv, 1024, 0) == ERROR)
